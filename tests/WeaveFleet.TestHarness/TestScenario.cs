...
public sealed class TestScenarioBuilder
{
    ...
    /// <summary>
    /// Enqueue a response that includes a question tool call for the next prompt.
    /// Emits: session.status(busy) → message.updated(user) → message.part.updated(user text)
    ///      → message.updated(assistant) → message.part.updated(question tool) → session.idle
    /// </summary>
        ...
        TimeSpan? delay = null)
    {
        var userMessageId = $"{messageId}-user";
        var userPartId = $"{userMessageId}-part-1";
        var toolPartId = $"{messageId}-tool-1";
        var toolCallId = $"{messageId}-call-1";
        var responseDelay = delay ?? TimeSpan.FromMilliseconds(10);

        var questionOptions = options.Select(o => new { label = o.Label, description = o.Description }).ToArray();
