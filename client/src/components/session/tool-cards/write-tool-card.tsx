...
import { getLanguageFromPath } from "@/lib/tool-card-utils";

interface WriteToolCardProps {
  part: AccumulatedPart & { type: "tool" };
}

export function WriteToolCard({ part }: WriteToolCardProps) {
  ...
  const input = state?.input as { filePath?: string; content?: string } | undefined;
  ...
  const hasExpandableContent = !isRunning && (content || error);
    ...
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger asChild disabled={!hasExpandableContent}>
        ...
        >
          ...
          <FilePlus2 className="h-3 w-3 shrink-0 text-green-400" />
          <span className="flex-1 truncate">{shortPath}</span>
          <span className="font-mono text-muted-foreground/50 shrink-0">write</span>
      ...
      <CollapsibleContent>
        <div className="mt-1 mb-1 ml-4 rounded-md border border-border/60 border-l-2 border-l-green-500/60 overflow-hidden">
          {/* Header bar */}
