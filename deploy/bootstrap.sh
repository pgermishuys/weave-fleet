...
# Disable tracing for secret handling — never echo or log secret values
set +x

FLEET_ENV_TMP="$(mktemp)"
TMPFILES+=("$FLEET_ENV_TMP")
chmod 600 "$FLEET_ENV_TMP"

# Write non-secret config using a literal heredoc (no shell expansion)
cat <<'ENVEOF' > "$FLEET_ENV_TMP"
# Weave Fleet runtime configuration
# Generated by bootstrap.sh — do not edit manually; re-run bootstrap to update
...
# Transfer via scp (never pass secrets through SSH command args)
scp_to_remote "$FLEET_ENV_TMP" "~/fleet.env.tmp"

# Atomic install on remote: 600 perms, fleet:fleet ownership
ssh_remote "sudo install -m 600 -o fleet -g fleet ~/fleet.env.tmp /opt/fleet/data/fleet.env && rm -f ~/fleet.env.tmp"

log "fleet.env provisioned at /opt/fleet/data/fleet.env (permissions: 600, owner: fleet:fleet)"

# Re-enable tracing if it was enabled before (generally not in CI, but be safe)
# set -x  # intentionally not restored — no need to trace after this point
