...
- [x] 9. End-to-end check (done 2026-09-12)
  ...
  - **As run**: Fleet from source (Debug, `ASPNETCORE_ENVIRONMENT=Development`, `127.0.0.1:5001`) with scratch `HOME`, `XDG_*`, `DOTNET_CLI_HOME` and database, serving a fresh client build from `wwwroot`. The session ran in a throwaway five-file TypeScript repo added as the scratch Fleet's only workspace root. The model was Claude Sonnet 5 through the user's GitHub Copilot login (their OpenCode `auth.json`, copied into the scratch data dir with their OK and deleted afterwards). The user's own OpenCode config wasn't loaded, so `visualize` wasn't available and only the `fleet_canvas_*` tools could draw. Prompts went through the real composer, driven by Playwright. `~/.weave/fleet.db` and its legacy backup were unchanged afterwards.
    1. "Draw me a diagram of how creating an order flows through the code in src/…", with no mention of canvases. The model called `fleet_canvas_open` on its own. The "Order Creation Flow" tab appeared mid-turn and took focus (11 boxes, v1). `canvas-agent-drawn-light.png`.
    2. "Change the diagram: publishing should write to an outbox table…". The model patched (v2), read the canvas, spotted a duplicate edge it had made, and patched again (v3). The same tab updated in place, with the same canvas id and no second tab.
    3. After a reload the canvas was still there at v3, with the outbox boxes, in light and dark. `canvas-agent-reloaded-light.png`, `canvas-agent-revised-dark.png`.
    4. Closing the tab removed it from `GET …/canvases`, and it stayed closed after a reload. "Show me that order flow diagram again." made the model call `fleet_canvas_focus`, which reopened the same canvas (still v3) and focused it in about 10 s. `canvas-agent-reopened-light.png`.
    - Every bridge call returned 200 (open, patch ×2, read, focus).
  - **Seen along the way, not fixed here**:
    - A tall diagram (11 boxes, top to bottom) is fitted into the narrow panel at a small zoom, so labels get small. The boxes are also narrower (Vue Flow's 150 px default) than the 172 px dagre lays out, so longer labels wrap and details are cut off. Worth a look when editing arrives: a minimum fit zoom, or boxes sized to their labels.
    - Tool calls show as generic cards ("Fleet_canvas_open fleet_canvas_open"), as planned. The plugin's card title (`Flow · +1 box · v2`) isn't shown yet.
    - The session header kept showing "Working" after turns ended while the status bar said IDLE. That's the known pooled-OpenCode status bug, not a canvas issue.
    - The event relay logged one `Cannot bind pooled OpenCode session to the active lease` for two other Fleet session ids in the same directory (not the E2E session). That's pooling, not canvases. Canvas events weren't affected.
