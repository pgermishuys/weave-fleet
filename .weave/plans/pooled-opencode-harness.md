...
- [ ] 1. Add feature flag `PooledOpenCodeHarness`
...
- [ ] 2. Codify verified SSE event scoping
...
- [ ] 3. Implement `PooledOpenCodeInstanceRegistry`
...
- [ ] 4. Implement `SseEventDemultiplexer`
...
- [ ] 4b. Implement `PoolDemuxBindingTable`
...
- [ ] 5. Extract process ownership from `OpenCodeHarnessSession`
...
- [ ] 6. Refactor `OpenCodeHarnessRuntime.SpawnAsync` for pooled mode
...
- [ ] 7. Refactor `OpenCodeHarnessRuntime.ResumeAsync` for pooled mode
...
- [ ] 8. Command routing with explicit OpenCode session ID
...
- [ ] 9. Process lifecycle: ref-count + idle TTL
...
- [ ] 10. Crash handling & automatic recovery
...
- [ ] 11. Working directory isolation enforcement
...
- [ ] 12. Credentials/env boundary enforcement
...
- [ ] 13. Stop/Delete/Resume semantics
...
- [ ] 17. Integration tests — full isolation matrix
...
- [ ] 18. Unit tests — registry, demux, lifecycle
...
- [ ] 19. Rollout: feature flag defaults & documentation
...
- [ ] 21. Early child event drop race mitigation
...
- [ ] 22. Backward compatibility — ensure non-pooled mode unchanged
...
- [ ] Feature flag OFF → existing behavior verified by regression tests
- [ ] Feature flag ON → pooled behavior verified by isolation matrix tests
- [ ] No frontend changes required (verified by unchanged API contracts)
- [ ] Telemetry/metrics visible in dev environment
- [ ] Process count under 5-session load: 1 (pooled) vs 5 (non-pooled)
- [ ] Crash recovery: kill process → all sessions recover within 5s
- [ ] Slash commands route correctly in pooled mode (verified by integration test)
- [ ] Security audit: unattributable SSE events never reach Fleet consumers (verified by SecurityAuditTests)
- [ ] Security audit: no plaintext credentials retained after hash computation (verified by CredentialHasher tests)
- [ ] Security audit: binding table rejects stale/unknown/mismatched events (verified by PoolDemuxBindingTable tests)
- [ ] Security audit: pool diagnostics endpoint returns 403/redacted for non-admin
- [ ] Security audit: resume verifies user ownership
