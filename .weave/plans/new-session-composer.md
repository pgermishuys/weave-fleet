...
- [x] 1.7 Live check — done 2026-09-13: 35/36 in six runs (3 dark, 3 light); page: https://claude.ai/code/artifact/996dc632-375d-40d2-870d-69b2afcd54f8
...
- **Fixed, first message not shown** (was the blocker): after Enter the session opens without the message; it appears only after a reload. The agent gets it (`prompt_async` 204, stored in OpenCode). Cause: `CreateSessionAsync` hands `InitialPrompt` to `SpawnAsync`, and the orchestrator broadcasts the user message (`BroadcastUserMessageAsync`, live only, not persisted) during the create request, before the page subscribes. The page's snapshot (DB) is taken before OpenCode's echo is stored, and the relay suppresses that echo from broadcast (`HarnessEventRelay.ShouldSuppressUserEcho`). Pre-existing for GitHub starts and automations. Seeding the client registry (2.2 as written) isn't enough: `ActivityStream` clears optimistic prompts when assistant content arrives, so the message would vanish again once the agent replies.
  ...
  - Done (`19d8353`). One correction to the diagnosis: a live OpenCode session's snapshot comes from OpenCode (`OpenCodeSessionMessageProxy`), not Fleet's DB, so the proxy now adds saved prompts OpenCode hasn't stored yet (same id; dropped once OpenCode has it or anything newer). Orchestrator + proxy tests.
  - The re-run found a second, client-side cause (`82d920e`): the hub sends `message.updated` with the harness's raw payload (no `parts`), and `applyMessageLifecycle` mapped `payload.parts` unconditionally. Arriving before the snapshot, that event was replayed on top of it and the throw discarded the snapshot: a blank session until reload, 5 of 9 full runs. Updates without parts now keep existing parts. The raw-payload mismatch in the hub remains (follow-up).
...
- **Noticed, not changed**: the message shifts ~235 px left when the session page opens, because the right panel appears (1.5 hides it on the new page). The header says "Working" at once, but the row and status bar say idle until the harness reports busy (the known status lag). In the phone menu the sessions panel is wider than the drawer, so counts and row labels spill over the backdrop (pre-existing; fixed in P.1).
- **Found, not from this change**: at 400 px every session page is unusable. The right panel takes almost the whole width and the conversation is 64 px wide, with the Stage 1 build too and after a reload (`.poc-runtime/pw-phone-session.mjs`). Fixed separately: P.1 below.

## Phone session layout (found in Stage 2)

- [x] P.1 The session page at 400 px — branch `fix/phone-session-layout`
  - **What**: At 400 px every session page was unusable: the right panel kept its 360 px column and the conversation got 64 px (Stage 1 build and older too). The same happened on narrow desktop windows: 70 px at 800 px, 294 px at 1024 px with the sidebar open. Now the panel is a column only while the conversation keeps 480 px beside it (`MIN_CONVERSATION_WIDTH`, measured on the workspace sheet), and its width is clamped to that room. Otherwise (always on phones) it's a sheet over the conversation, closed until opened from the header's panel button, the palette's Show Right Panel, or anything that reveals the panel (a visual's expand, a context chip); it closes on navigation. The sidebar store holds `rightPanelAsSheet` / `rightPanelSheetOpen`; `useSidebarMobile()` gains `showRightPanel` / `hideRightPanel` / `toggleRightPanel`. The session header clears the fixed menu button on phones. The phone menu drawer is `min(340px, 100% − 56px)` and the sessions panel fills it (it was 280 px wide inside a 280 px drawer next to the 48 px rail, so counts and row labels spilled over the backdrop).
  - **Files**: `client/src/components/layout/AppShell.vue`, `ContextPanel.vue`, `RightPanelSheetButton.vue`, `client/src/stores/sidebar.ts`, `client/src/composables/use-sidebar-mobile.ts`, `SessionsV2RightPanel.vue`, `BoardRightPanel.vue`, `CanvasHost.vue` (`widenable`), `SessionDetailHeader.vue`, `routes/sessions.$id.tsx`, reveal call sites (`ActivityStream.vue`, `SessionContextChips.vue`, `use-commands.ts`)
  - **Acceptance**: live at 400, 700, 800, 1024 and 1280 px in both themes (`.poc-runtime/pw-phone.mjs`): conversation gets the whole sheet or keeps ≥ 480 px, panel opens, fits and closes with Esc, menu content stays inside the drawer, column collapse/expand unchanged at 1280 px.
