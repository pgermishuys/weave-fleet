...
- **Found, not from this change**: at 400 px every session page is unusable. The right panel takes almost the whole width and the conversation is 64 px wide, with the Stage 1 build too and after a reload (`.poc-runtime/pw-phone-session.mjs`). Needs its own fix (AppShell / right panel at phone width).
